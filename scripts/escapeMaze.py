...
def movingForward(vel_msg, velocity_publisher, t0, current_distance, distance, speed, forwardSpeed, front):
	...
	while(current_distance < distance):
		...
		current_distance = speed*(t1-t0)#calculates distance#Stop the robot when the front distance from obstacle is smaller than 1.0  
	if (front < 1.0):
...
def escapeMaze():
   	...
   	distance = [0.10, 0.20, 0.29]   #set all the linear and angular motion of each dimension to zero        
			...
			if (no_front_wall == True) and (front > 1.0):
				...
				if (left > 0.5) and ((right > 3.0) or (top_right > 3.0)):     
					...
					if front < 1.0:
						...
						turnCCW(vel_msg, velocity_publisher, t0, 0, 3, 90)# The node will stop when press control + C
